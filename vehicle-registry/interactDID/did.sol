...
contract LBL_IoTeX_DID {
    ...
    mapping(string => string) imeiToDID;

    event CreateDID(string id, string didString);
    event UpdateHash(string didString, bytes32 hash);
    event UpdateURI(string didString, string uri);
    event DeleteDID(string didString);

    function createDID(bytes32 hash, string uri, string imei) public {
        string memory resultDID = generateDIDString();
        require(!dids[resultDID].exist, "did already exists");
        if (bytes(imei).length > 0) {
            string memory imeiString = toLower(imei);
            imeiToDID[imeiString] = resultDID;
        }
        dids[resultDID] = DID(true, hash, uri);

        emit CreateDID(addrToString(msg.sender), resultDID);
    }
    ...
    function remindDIDString() public view returns (string){
        return generateDIDString();
    }

    function getDocumentUriFromIMEI(string imei) public view returns (string) {
        string memory imeiString = toLower(imei);
        require(dids[imeiToDID[imeiString]].exist, "did does not exist");
        return dids[imeiToDID[imeiString]].uri;
    }
